    ...
    unsigned int timeout_ms)
{
    ...
    manager.setCompleteCallback([&results](const StreamKey& key, const DABEnsemble& ens) {
...
struct EnsembleDiscovery::Impl {
        ...
        , start_time(std::chrono::steady_clock::now())
    {
        ...
        manager.setCompleteCallback([this](const StreamKey& key, const DABEnsemble& ens) {
            DiscoveredEnsemble de;
            de.ip = key.ip;
            de.port = key.port;
            de.eid = ens.eid;
