#pragma once
// Shared CRC utilities for DAB/DVB processing
// - CRC-16 CCITT (polynomial 0x1021) - FIB, packet mode, ETI
// - CRC-32 DVB (polynomial 0x04C11DB7) - TS PSI tables
// - FireCode CRC - DAB+ superframe sync
...
inline const uint16_t crc16_ccitt_table[256] = {
...
// CRC-16 CCITT: init=0xFFFF, no final XOR (for FIB, ETI headers)
...
// CRC-16 CCITT with final XOR (for packet mode data groups)
inline uint16_t crc16_ccitt_xor(const uint8_t* data, size_t len) {
    return crc16_ccitt(data, len) ^ 0xFFFF;
}

// DVB CRC-32 lookup table (polynomial 0x04C11DB7)
inline const uint32_t dvb_crc32_table[256] = {
...
// DVB CRC-32: init=0xFFFFFFFF, no final XOR (for PSI tables)
inline uint32_t dvb_crc32(const uint8_t* data, size_t len) {
...
// Used for DAB+ superframe synchronization
inline const uint16_t firecode_table[256] = {
...
// FireCode CRC check - returns true if bytes 0-1 match CRC of bytes 2-10
// Used for DAB+ superframe synchronization
inline bool firecode_check(const uint8_t* buf) {
    uint16_t crc = 0;
    const uint8_t* ptr = buf + 2;
    for (int i = 0; i < 9; i++) {
        crc = (crc << 8) ^ firecode_table[((crc >> 8) ^ ptr[i]) & 0xFF];
    }
    return ((crc >> 8) == buf[0]) && ((crc & 0xFF) == buf[1]);
}

// CRC-16 CCITT verification - returns true if CRC over data (including CRC bytes) is 0
// This is for verifying data where CRC is appended at the end
inline bool crc16_ccitt_verify(const uint8_t* data, size_t len) {
    uint16_t crc = 0xFFFF;
    for (size_t i = 0; i < len; i++) {
        crc = (crc << 8) ^ crc16_ccitt_table[((crc >> 8) ^ data[i]) & 0xFF];
    }
